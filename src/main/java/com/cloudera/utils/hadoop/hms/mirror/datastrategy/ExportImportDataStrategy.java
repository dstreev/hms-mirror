...
//            dsEIADI.setConfig(config);
            ...
            if (TableUtils.isACID(let)) {
                if (getConfigService().getConfig().getCluster(Environment.LEFT).getLegacyHive() != config.getCluster(Environment.RIGHT).getLegacyHive()) {
                    rtn = Boolean.FALSE;
    ...
    public Boolean buildOutDefinition(TableMirror tableMirror) {
        ...
        if (TableUtils.isACID(let) &&
                !getConfigService().getConfig().getCluster(Environment.LEFT).getLegacyHive()
                        .equals(getConfigService().getConfig().getCluster(Environment.RIGHT).getLegacyHive())) {
